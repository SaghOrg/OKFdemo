...
| **Credit note netting threshold** (VAR-005, Finance's manual Excel net-down) | **None recorded.** No threshold of any value appears anywhere in this knowledge base before this record — the manual net-down is described in `DOC-05`, `/concepts/variances/var-005-credit-notes-absent.md` and `/concepts/tables/oms-prod-credit-note.md` without a materiality figure. The word used in the capture was "now," which implies a prior value; if one existed, this corpus never held it. | **INR 25,000 per credit note document.** Documents at or below the threshold are not netted individually. Applied at document (header) grain, not line grain. | 2026-08-26 (captured; see Notes) | No earlier source — nothing to supersede. | **Verbal confirmation by Shalini Iyer (Head of Finance Systems, BCPL) — captured live into this knowledge base on 26-Aug-2026, with no `_sources/` artifact behind it.** The `sources` on the decision record are context provenance only (`DOC-05` the netting process, `EM-063` the 1,206-document population, `T-08` the Klarissen deadline); none of them states the threshold. **Two caveats carried deliberately:** 26-Aug-2026 is the capture date, *not* necessarily the date Finance decided — no decision date was stated and none is invented; and the threshold sits essentially at the mean document value of the validated FY26 population (INR 3,11,20,000 / 1,206 ≈ INR 25,800), which nobody has assessed for impact. See `/decisions/20260826-var005-credit-note-netting-threshold.md`. |
