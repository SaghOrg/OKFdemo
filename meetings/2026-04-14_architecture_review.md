...
- Product dimension: SCD1 (overwrite on every attribute change). Karthik reasoned that SKU attributes are effectively static after creation (brand, category, pack size, UoM, MRP, HSN remain constant), so versioning adds complexity without value. The only material changes are active/delisting flags, which are caught by DQ rules instead. No pushback; decision accepted in-meeting.

- Customer dimension: **Open, pending performance data.** Tension between Farida's load window constraint and the integrity risk that SCD1 (overwrite) restates history on territory reassignment. If a distributor moves from Territory A to Territory B in April, SCD1 causes all of last year's sales to retroactively report against Territory B. Karthik proposed SCD2 (effective dating, versioned rows), but Farida raised the window cost—month-end nights already run 3+ hours; SCD2 would add processing for territory changes (realignments move 50–60+ distributors in a single night, 3–4 times a year). Karthik committed to measure actual load cost on DEV; Farida to send her position separately in writing. No decision will be taken until both numbers are available at the May sign-off meeting.

- Date dimension itself is fine (clean calendar with fiscal columns through 2031), but the **key derivation is wrong**: `DATE_KEY` is derived from `INVOICE_HEADER.CREATED_TS`, which the ORION app server writes in UTC. Any invoice raised after 18:30 IST on the last day of a month lands in the next month's key, causing month-end revenue to shift between months. Ishaan found it during variance root-cause analysis. Ani noted this was solved the same way in a 2017 OBIEE report (the Despatch Analysis report); Priya to confirm that report uses `INVOICE_DT` instead. Impact: roughly 90 lakh rows misplaced in FY26, netting out at year level but material at month and quarter level. Fix is one-line: `DATE_KEY` from `INVOICE_DT` (IST date, not UTC timestamp). Requires 9-month historical reload. This is VAR-002.
